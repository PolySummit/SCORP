...
class ParamGroup:
    ...
    def extract(self, args=None):
        group = GroupParams()
        for arg in vars(self if args is None else args).items():
            _k, _v = arg
            _k = _k[1:] if _k.startswith("_") else _k
            if _k in vars(self) or ("_" + _k) in vars(self):
                setattr(group, _k, _v)
        return group
...
class OptimizationParams(ParamGroup):
    def __init__(self, parser=None):
        ...
        self.lambda_isotropic = 0.0001

        self.opacity_cull = 0.5
        self.max_screen_size = 0.2

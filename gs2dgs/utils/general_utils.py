...
def PILtoTorch(pil_image: Image.Image, resolution=None, scale=255.0):
...
def TorchtoPIL(
    ...
    clip_max: float | None = 1,
):
    tensor_image = tensor_image.permute(1, 2, 0)
    tensor_image = tensor_image.cpu().numpy()
    tensor_image = (np.clip(np.nan_to_num(tensor_image), 0., clip_max) * scale).astype(dtype)
    return Image.fromarray(tensor_image.squeeze())
